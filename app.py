...
if not os.path.exists(DB_FILE):
    df = pd.DataFrame(columns=["Item Name", "Category", "Quantity", "Min Threshold"])
...
with st.sidebar.form(key="add_form", clear_on_submit=True):
    ...
    new_qty = st.number_input("Current Quantity", min_value=0, value=1, step=1)
    new_min = st.number_input("Alert Threshold (Low Stock level)", min_value=0, value=2, step=1)
    
    submit_button = st.form_submit_form_button = st.form_submit_button(label="Add Item")

if submit_button and new_name:
    # Check if item already exists
    if new_name.strip().lower() in df["Item Name"].str.lower().values:
        st.sidebar.error(f"'{new_name}' already exists! Update its stock in the main dashboard.")
    else:
        new_row = pd.DataFrame([[new_name.strip(), new_cat, new_qty, new_min]], columns=df.columns)
...
if df.empty:
    ...
    # Highlight items that are low or out of stock
    ...
    col2.metric("Items to Restock", low_stock_count, delta_color="inverse")
    
    # Display the interactive dataframe editor
    ...
    # Save changes if user interacts with the editor data table
    if not edited_df.equals(df):
        # Drop the Status column before saving
